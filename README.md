...
![Codex QQ Skin 窗口级 QQ 秀界面](./assets/qq-show-window-layout.png)

右侧 QQ 秀是与 Codex 主界面并列的第三栏，会跟随窗口宽度自适应，并可折叠为 40px 窄栏；聊天区域同步回流，不会在窗口内部悬浮遮挡内容。
...
- `QQ`：使用固定的蓝银 QQ 2007 外框、左侧栏、三栏布局和可收纳的窗口级 QQ 秀，不受用户图片配色影响。
...
- 自动打开 Codex 原生左侧栏，并在窗口最右侧挂载独立 QQ 秀，形成左侧项目、中间对话、右侧 QQ 秀的三栏布局。
- QQ 秀包含“对方形象”和“我的形象”两张资料卡、在线状态与快捷操作区；收起后只保留 40px 窄栏，并自动释放空间给聊天区域。
